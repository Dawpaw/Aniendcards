...
def create_media_endcard(
    db: Session, endcard: schemas.EndcardsCreate, episode_id: int, artist_id: int
):
    db_endcard = models.Endcards(
        **endcard.model_dump(), episode_id=episode_id, artist_id=artist_id
    )
    db.add(db_endcard)
    db.commit()
